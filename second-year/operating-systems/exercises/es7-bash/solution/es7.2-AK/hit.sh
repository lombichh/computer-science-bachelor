...
if [[ $# -lt 3 ]]; then
    ...
    echo  "Usage: $0 str extension dir1…dirN" 
    exit 1
fi
...
#faccio scorrere gli argomenti in ingresso per avere solo i direttori (i.e. butto via i primi 2 argomenti)
shift
shift
...
for dir in "$@"; do
    ...
    "$recursive_command" "$str" "$extension" "$dir" $$
done

#stampa del numero totale dei file trovati:
