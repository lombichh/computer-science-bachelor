		...
		for (int i = 1; i < args.length; i = i + 2) {
			...
			try {
				...
				if (portCorr <= 1024 || portCorr > 65535) {
					...
					//controllo che fra le precedenti coppie file porta non ci sia la porta args[i+1]
					if(i>2){	// al primo giro non controllo la porta precedente
						int j=i-1;
						while(j>1){ // controllo tutte le porte precedenti
							portprec=Integer.parseInt(args[j]);
							if (portprec==portCorr){
								System.out.println("La porta " + portCorr + " non è valida poichè già in uso e il file " + fileCorr.getName() + " non verrà elaborato.");
								for(int k = i; k < (args.length-2); k++){
									args[k]=args[k+2];
								}
								args[args.length-1]=null;
								args[args.length-2]=null;
					...
					}else {	// al primo giro non controllo la porta precedente e aggiungo 
						System.out.println("Aggiunto il seguente SwapServer - File: " + fileCorr + ",\t porta: " + portCorr);
						validArgs++;
					}
