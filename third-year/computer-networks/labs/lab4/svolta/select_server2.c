    ...
    for (;;) {
        ...
        if (FD_ISSET(listenfd, &rset)) {
            ...
            if (fork() == 0) { /* processo figlio che serve la richiesta di operazione */
                ...
                while((nread=read(connfd,&nome_file,sizeof(nome_file)))>0) {
                    ...
                    if (fd_file<0){
                        ...
                        write(connfd, &zero, 1); // Invio terminazione file: zero binario
                        close(fd_file);   // Libero la risorsa sessione file
                    }//else 
                    if (nread < 0) {// problema di errore read
                        printf("Ricevuto errore read \n"); 
                        break;
                    }
                }// while
                close(connfd); 
