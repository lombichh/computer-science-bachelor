...
function textAreaOnMouseOver(event) {
	event.target.style.background = "yellow";
}

function textAreaOnMouseOut(event) {
	event.target.style.background = "white";
}

function textAreaOnKeyUp(event) {
	const remainedChars = 64 - parseInt(event.target.value.length);

	if (remainedChars < 0) {
		event.target.value = 
					event.target.value.substring(0, event.target.value.length - 1);
	} else textAreaOutput.innerHTML = "Caratteri rimasti: " + remainedChars;
}

const textArea = document.getElementById("textarea");
textArea.onmouseover = textAreaOnMouseOver;
textArea.onmouseout = textAreaOnMouseOut;
textArea.onkeyup = textAreaOnKeyUp;